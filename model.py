...
class HGSAGELayer(nn.Module):
    ...
    def __init__(self, meta_paths, in_size, out_size, dropout, aggregator_type):
        ...
        # One GraphSAGE layer for each meta path based adjacency matrix
        self.sage_layers = nn.ModuleList()
        for i in range(len(meta_paths)):
            self.sage_layers.append(SAGEConv(in_size, out_size, aggregator_type,
                                             dropout, activation=F.elu))
    ...
    def forward(self, g, h):
        ...
        for i, meta_path in enumerate(self.meta_paths):
            ...
            semantic_embeddings.append(self.sage_layers[i](new_g, h).flatten(1))     # 采样和聚合
